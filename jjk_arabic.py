...
def get_arabic_chapters(url):
    ...
    pattern = re.compile(r'https://3asq.org/manga/jujutsu-kaisen/\d+/')
...
def download_chapter_ara(chapter_number):
    ...
    url = f'https://3asq.org/manga/jujutsu-kaisen/{chapter_number}/'
    response = requests.get(url)
    if response.status_code != 200:
...
arabic = 'https://3asq.org/manga/jujutsu-kaisen/'
